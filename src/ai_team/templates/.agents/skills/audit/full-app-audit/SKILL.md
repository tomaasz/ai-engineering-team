...
Generate the final audit report into `docs/AUDIT.md` (or specified output path) using this exact structure:

```markdown
# 360° Application Audit Report

**Audit Date**: [YYYY-MM-DD]  
**Overall System Health**: [A / B / C / D / F] (Score: 0–100%)  
**Executive Summary**: [2-3 concise paragraphs summarizing the system status and critical risks]

---

## Findings Matrix

| ID | Category | Severity | File:Line | Finding Summary | Recommended Fix |
|---|---|---|---|---|---|
| SEC-01 | Security | CRITICAL | src/auth.js:42 | Hardcoded JWT secret key | Move to environment variable with startup validation |
| ARC-01 | Architecture | MEDIUM | src/db/repo.js:105 | N+1 query loop on user fetch | Use SQL JOIN or dataloader batching |
...
- **Impact**: Detailed explanation of the risk, exploitability, and business consequence.
- **Proof of Concept / Evidence**: Code snippet illustrating the issue.
- **Remediation**: Exact, minimal replacement code resolving the vulnerability.

---

## Action Plan
- **Phase 1 (Immediate / P0)**: Critical security fixes and data corruption risks.
- **Phase 2 (Short-term / P1)**: High-priority bugs, missing error handling, test gaps.
- **Phase 3 (Medium-term / P2)**: Architectural refactoring, performance tuning, and technical debt reduction.
```
