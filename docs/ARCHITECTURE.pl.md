...
Każdy przebieg zapisuje swój base ref, branch, prompt, wyjście etapów, stderr, logi kontroli i kanoniczny `.ai/runs/<run-id>/result.json`. Domyślnie używany jest wygenerowany branch. Nie zachodzi żaden automatyczny commit, push, merge ani deployment.
